    ...
    <div className="rounded-2xl border border-[color:var(--line-soft)] bg-[color:var(--wash-0)] p-4 text-sm shadow-[var(--shadow-soft)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="font-display text-lg text-[color:var(--ink-900)]">{focusOption.label}</p>
          <p className="text-sm text-[color:var(--ink-600)]">{focusOption.description}</p>
        </div>
        <button
          type="button"
          onClick={handleClear}
          className="min-h-11 rounded-full border border-[color:var(--line-soft)] bg-[color:var(--wash-0)] px-4 py-2 text-sm font-semibold uppercase tracking-[0.2em] text-[color:var(--ink-800)] transition hover:border-[color:var(--ink-800)]"
        >
          Clear focus
        </button>
      </div>
      <p className="mt-3 text-sm font-semibold uppercase tracking-[0.2em] text-[color:var(--ink-500)]">
