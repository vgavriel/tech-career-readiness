...
const ensureLessonReady = async (page) => {
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
  await expect(
    page.getByRole("button", { name: /mark complete|mark incomplete/i })
  ).toBeVisible();
};

const resetToIncomplete = async (page) => {
  await expect(
    page.getByRole("button", { name: /mark complete|mark incomplete/i })
  ).toBeVisible();
...
test.describe("progress flows", () => {
  test("persists authenticated progress and updates the continue CTA", async ({
    page,
  }) => {
  ...
  test("merges guest progress into the account on sign-in", async ({ page }) => {
    ...
    await ensureLessonReady(page);
    await page.evaluate(() =>
      localStorage.removeItem("tcr-guest-progress")
    );
    ...
    const stored = await page.evaluate(() =>
      localStorage.getItem("tcr-guest-progress")
    );
